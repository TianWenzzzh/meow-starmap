...
class V30GoldenFreshnessTests(unittest.TestCase):
    def test_make_v30_baseline_check_clean(self) -> None:
        r = subprocess.run(
            [sys.executable, str(REPO / "tools" / "make_v30_baseline.py"),
             "--check"],
            capture_output=True, text=True, cwd=str(REPO))
        self.assertEqual(
            r.returncode, 0,
...
class V32GoldenFreshnessTests(unittest.TestCase):
    def test_make_v32_baseline_check_clean(self) -> None:
        r = subprocess.run(
            [sys.executable, str(REPO / "tools" / "make_v32_baseline.py"),
             "--check"],
            capture_output=True, text=True, cwd=str(REPO))
        self.assertEqual(
            r.returncode, 0,
